...
app.get("/account", async (req, res) => {
  try {
    ...
    if (checkUserExist(userId, accessToken, res)) {
...
async function getAllPlaylist(res, accessToken) {
  try {
    ...
    if (all_playlists.data.error) {
      throw new Error("Error: " + all_playlists.data.error);
    }
    return all_playlists;
  } catch (error) {
    console.error("Une erreur s'est produite lors de la récupération de la liste des playlists:", error.message);
    throw error;
  }
